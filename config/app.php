...
return [
    ...
    // 应用映射（自动多应用模式有效）
    'app_map'          => [
        Env::get('easyadmin.admin', 'cadminb') => 'admin',
        Env::get('easyadmin.ajax', 'ajax') => 'ajax',
    ],
    // 后台别名
    'admin_alias_name' => Env::get('easyadmin.admin', 'cadminb'),
    // 域名绑定（自动多应用模式有效）
    'domain_bind'      => [],
    // 禁止URL访问的应用列表（自动多应用模式有效）
    'deny_app_list'    => ['common'],
    // 异常页面的模板文件
    'exception_tmpl'   => Env::get('app_debug') == 1 ? app()->getThinkPath() . 'tpl/think_exception.tpl' : app()->getAppPath() . '/index/view/index/404.html',
    // 跳转页面的成功模板文件
